...
def execute(motorSpeed,simulationTime,clockTime):
    ...
    while True:
        ...
        thisTime = time.time()
        a = startTimeClock+clockTime
        if clockTime>0 and thisTime>a: break
        time.sleep(0.1)
    STOP()
...
def init():
    ...
    if clientID != -1:
        ...
        if res == vrep.simx_return_ok: # Remote function call succeeded
            ...
            ret_sf,  ultraSonicSensorFront = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor5',vrep.simx_opmode_oneshot_wait)
            blockHandleArray = []
